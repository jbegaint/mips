...
# Usage string
USAGE="Usage: `basename $0` -e <executablefile> [-b] [-p] [-d] [-q] [-v] [-o <summaryfile>] [-x] [-w] [-c] [-s <skip_file>] <args_testfiles>"
...
printHelp()
{
	...
	echo "	1/ a file testfile.info : optional : information on the test. See README"
	echo "	2/ a file testfile.res  : only if the test passes ; contains the awaited output"
	echo 
	...
	echo ""
	echo " Arguments :"
	echo "	simpleUnitTest [-h] -e <executablefile> -t <list of test files> [-b] [-o <summaryfile>] [-x] [-w]"
	echo " Where :"
	...
	echo "	-x				: if used, -x is passed to diff when comparing the results. See man diff"
...
# - the .info script containing the awaited exit code, and an optional comment
# - the .res file containing the awaited output for the test
# - the .out file that will be generated
# each test file is passed to the command (4 args)
echoTestEnvironnment()
{
	if [ $# -ne 4 ]
	then
	...
	if [ ! -r "$2" ]
	then
		echoOutputHighlight "	info file:	$2	Not Readable"
	else
		echoOutputQuiet		"	info file:	$2	(OK)"
	fi	
		
	if [ ! -r "$3" ]
	then
		echoOutputHighlight "	awaited result file:	$3	Not Readable"
	else
		echoOutputQuiet		"	awaited result file:	$3	(OK)"
	fi		
...
executeTest()
{
	...
	if [ -r "$info_file_name" ]
	then
		# $info_file_name should be a script
		# that sets two variables : TEST_RETURN_CODE and TEST_COMMENT
		source "$info_file_name"
	fi
	...
	echoTestEnvironnment $test_file_name $info_file_name $res_file_name $out_file_name
	...
	then
		...
		echoOutputHighlight "--> Test output is not correct. Check $res_file_name and $out_file_name for details."
...
#		diff --context=2 $DIFF_OPTIONS $out_file_name $res_file_name | echoOutputQuiet
	...
	if [ $TEST_OK == $TRUE ]
	then
		echoOutputHighlight "TEST PASSES    :  $test_file_name"	
		TEST_PASSED="$TEST_PASSED $test_file_name"
		TEST_PASSED_COUNT=`expr $TEST_PASSED_COUNT + 1 `
		return 0
	fi
		
	echoOutputHighlight "===> TEST FAILS   : $test_file_name "	
...
echoOutput "PASSED  TESTS:"
echoOutput "	$TEST_PASSED"

echoOutput "FAILED  TESTS:"
...
echoOutput "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
echoOutput "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
echoOutput "TEST PASSED  : $TEST_PASSED_COUNT    out of $TEST_COUNT "
echoOutput "TEST FAILED  : $TEST_FAILED_COUNT    out of $TEST_COUNT "
